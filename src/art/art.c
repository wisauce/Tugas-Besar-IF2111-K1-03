#include <stdio.h>
#include "../console.h"

void welcomeMessage()
{
    clearterminal();
    // Welcome message
    printf(" .+\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+. \n");
    printf("(                                                                                 )\n");
    printf(" )      __    __       _                                    _                    ( \n");
    printf("(      / / /\\ \\ \\ ___ | |  ___  ___   _ __ ___    ___      | |_  ___              )\n");
    printf(" )     \\ \\/  \\/ // _ \\| | / __|/ _ \\ | '_ ` _ \\  / _ \\     | __|/ _ \\            ( \n");
    printf("(       \\  /\\  /|  __/| || (__| (_) || | | | | ||  __/     | |_| (_) |            )\n");
    printf(" )       \\/  \\/  \\___||_| \\___|\\___/ |_| |_| |_| \\___|      \\__|\\___/            ( \n");
    printf("(                                                                                 )\n");
    printf(" )        ___                                             _                      ( \n");
    printf("(        / _ \\ _   _  _ __  _ __  _ __ ___    __ _  _ __ | |_                     )\n");
    printf(" )      / /_)/| | | || '__|| '__|| '_ ` _ \\  / _` || '__|| __|                   ( \n");
    printf("(      / ___/ | |_| || |   | |   | | | | | || (_| || |   | |_                     )\n");
    printf(" )     \\/      \\__,_||_|   |_|   |_| |_| |_| \\__,_||_|    \\__|                   ( \n");
    printf("(                                                                                 )\n");
    printf(" )                                                                               ( \n");
    printf("(                                                                                 )\n");
    printf(" \"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+.\"+. \"  \n\n");

    printf("--------Kelompok 3 K1---------                                                 \n");                      
    printf("       Welcome to Purrmart                                                      \n");
}

void thankYouLetter()
{
    clearterminal();
    printf("  _______  _    _            _   _  _  __ __     __ ____   _    _                       \n");
    printf(" |__   __|| |  | |    /\\    | \\ | || |/ / \\ \\   / // __ \\ | |  | |                      \n");
    printf("    | |   | |__| |   /  \\   |  \\| || ' /   \\ \\_/ /| |  | || |  | |                      \n");
    printf("    | |   |  __  |  / /\\ \\  | . ` ||  <     \\   / | |  | || |  | |                      \n");
    printf("    | |   | |  | | / ____ \\ | |\\  || . \\     | |  | |__| || |__| |                      \n");
    printf("    |_|   |_|  |_|/_/    \\_\\|_| \\_||_|\\_\\    |_|   \\____/  \\____/                       \n");
    printf("                                                                                        \n");
    printf("                                                                                        \n");
    printf("  ______  ____   _____      _____  _    _   ____    ____    _____  _____  _   _   _____ \n");
    printf(" |  ____|/ __ \\ |  __ \\    / ____|| |  | | / __ \\  / __ \\  / ____||_   _|| \\ | | / ____|\n");
    printf(" | |__  | |  | || |__) |  | |     | |__| || |  | || |  | || (___    | |  |  \\| || |  __ \n");
    printf(" |  __| | |  | ||  _  /   | |     |  __  || |  | || |  | | \\___ \\   | |  | . ` || | |_ |\n");
    printf(" | |    | |__| || | \\ \\   | |____ | |  | || |__| || |__| | ____) | _| |_ | |\\  || |__| |\n");
    printf(" |_|     \\____/ |_|  \\_\\   \\_____||_|  |_| \\____/  \\____/ |_____/ |_____||_| \\_| \\_____|\n");
    printf("                                                                                        \n");
    printf("                                                                                        \n");
    printf("  _____   _    _  _____   _____   __  __            _____  _______                      \n");
    printf(" |  __ \\ | |  | ||  __ \\ |  __ \\ |  \\/  |    /\\    |  __ \\|__   __|                     \n");
    printf(" | |__) || |  | || |__) || |__) || \\  / |   /  \\   | |__) |  | |                        \n");
    printf(" |  ___/ | |  | ||  _  / |  _  / | |\\/| |  / /\\ \\  |  _  /   | |                        \n");
    printf(" | |     | |__| || | \\ \\ | | \\ \\ | |  | | / ____ \\ | | \\ \\   | |                        \n");
    printf(" |_|      \\____/ |_|  \\_\\|_|  \\_\\|_|  |_|/_/    \\_\\|_|  \\_\\  |_|                        \n");
    printf("                                                                                        \n");
    printf("                                                                                        \n");
}

void artHelp()
{
    clearterminal();
    printf(" __    __   _______  __      .______   \n");
    printf("|  |  |  | |   ____||  |     |   _  \\  \n");
    printf("|  |__|  | |  |__   |  |     |  |_)  | \n");
    printf("|   __   | |   __|  |  |     |   ___/  \n");
    printf("|  |  |  | |  |____ |  `----.|  |      \n");
    printf("|__|  |__| |_______||_______|| _|      \n");
    printf("                                        \n");
    printf("\n");
}

void artRegister()
{
    printf(".______       _______   _______  __       _______.___________. _______ .______      \n");
    printf("|   _  \\     |   ____| /  _____||  |     /       |           ||   ____||   _  \\     \n");
    printf("|  |_)  |    |  |__   |  |  __  |  |    |   (----`---|  |----`|  |__   |  |_)  |    \n");
    printf("|      /     |   __|  |  | |_ | |  |     \\   \\       |  |     |   __|  |      /     \n");
    printf("|  |\\  \\----.|  |____ |  |__| | |  | .----)   |      |  |     |  |____ |  |\\  \\----.\n");
    printf("| _| `._____||_______| \\______| |__| |_______/       |__|     |_______|| _| `._____| \n");
    printf("                                                                                     \n");
    printf("\n");
}

void artLogin()
{
    printf(" __        ______     _______  __  .__   __. \n");
    printf("|  |      /  __  \\   /  _____||  | |  \\ |  | \n");
    printf("|  |     |  |  |  | |  |  __  |  | |   \\|  | \n");
    printf("|  |     |  |  |  | |  | |_ | |  | |  . `  | \n");
    printf("|  `----.|  `--'  | |  |__| | |  | |  |\\   | \n");
    printf("|_______| \\______/   \\______| |__| |__| \\__| \n");
    printf("                                             \n");
    printf("\n");
}

void welcomeMenuList()
{
    printf("\n=========================================\n");
    printf("               WELCOME MENU                \n");
    printf("=========================================\n");
    printf(" 1. START\n");
    printf(" 2. LOAD <filename>\n");
    printf(" 3. EXIT\n");
    printf(" 4. HELP\n");
    printf("=========================================\n");
}

// Menampilkan welcome help menu
void welcomeHelpMenu() 
{
    artHelp();
    printf("\n");
    printf("=====[ Welcome Help Menu PURRMART ]=====\n");
    printf("START -> Untuk masuk sesi baru\n");
    printf("LOAD -> Untuk memulai sesi berdasarkan file konfigurasi\n");
    printf("EXIT -> Untuk keluar dari program\n\n");
}

// Menampilkan login help menu
void loginHelpMenu() {
    artHelp();
    printf("\n");
    printf("=====[ Login Help Menu PURRMART ]=====\n");
    printf("REGISTER -> Untuk melakukan pendaftaran akun baru\n");
    printf("LOGIN -> Untuk masuk ke dalam akun dan memulai sesi\n");
    printf("LOGOUT -> Untuk keluar dari sesi\n");
    printf("EXIT -> Untuk keluar dari program\n\n");
}

void loginMenuList() {
    printf("\n=========================================\n");
    printf("               LOGIN MENU                \n");
    printf("=========================================\n");
    printf(" 1. REGISTER\n");
    printf(" 2. LOGIN\n");
    printf(" 3. LOGOUT\n");
    printf(" 4. EXIT\n");
    printf(" 5. HELP\n");
    printf("=========================================\n");
}

// Menampilkan main menu
void mainHelpMenu() 
{
    artHelp();
    printf("\n");
    printf("=====[ Main Help Menu PURRMART ]=====\n");
    printf("PROFILE -> Untuk melihat profil pengguna yang sedang login\n");
    printf("WORK -> Untuk bekerja dan mendapatkan penghasilan\n");
    printf("WORK CHALLENGE -> Untuk mengerjakan challenge khusus\n");
    printf("STORE LIST -> Untuk melihat daftar barang di toko\n");
    printf("STORE REQUEST -> Untuk meminta penambahan barang di toko\n");
    printf("STORE SUPPLY -> Untuk menambahkan barang dari permintaan ke toko\n");
    printf("STORE REMOVE -> Untuk menghapus barang dari toko\n");
    printf("CART ADD <nama> <jumlah> -> Untuk menambahkan barang ke keranjang belanja\n");
    printf("CART REMOVE <nama> <jumlah> -> Untuk menghapus barang dari keranjang belanja\n");
    printf("CART SHOW -> Untuk melihat isi keranjang belanja\n");
    printf("CART PAY -> Untuk membayar isi keranjang belanja\n");
    printf("HISTORY <jumlah> -> Untuk melihat riwayat pembelian pengguna\n");
    printf("WISHLIST ADD -> Untuk menambahkan barang ke wishlist\n");
    printf("WISHLIST SWAP <i> <j> -> Untuk menukar posisi wishlist ke-i dan ke-j\n");
    printf("WISHLIST REMOVE <i> -> Untuk menghapus wishlist posisi ke-i\n");
    printf("WISHLIST REMOVE -> Untuk menghapus wishlist berdasarkan nama\n");
    printf("WISHLIST CLEAR -> Untuk menghapus seluruh wishlist\n");
    printf("WISHLIST SHOW -> Untuk melihat wishlist saat ini\n");
    printf("LOGOUT -> Untuk keluar dari sesi pengguna saat ini\n");
    printf("SAVE <filename> -> Untuk menyimpan data program ke file\n");
    printf("EXIT -> Untuk keluar dari aplikasi\n");
    printf("HELP -> Untuk menampilkan panduan menu ini\n");
    printf("BIOWEAPON -> Untuk membuat senjata biologis rahasia\n");
}

void mainMenuList() 
{
    printf("\n=========================================\n");
    printf("               MAIN MENU                 \n");
    printf("=========================================\n");
    printf(" >>> PROFILE\n");
    printf(" 1. WORK\n");
    printf(" 2. WORK CHALLENGE\n");
    printf(" 3. STORE LIST\n");
    printf(" 4. STORE REQUEST\n");
    printf(" 5. STORE SUPPLY\n");
    printf(" 6. STORE REMOVE\n");
    printf(" 7. CART ADD <nama barang> <jumlah>\n");
    printf(" 8. CART REMOVE <nama barang> <jumlah>\n");
    printf(" 9. CART SHOW\n");
    printf("10. CART PAY\n");
    printf("11. HISTORY <jumlah>\n");
    printf("12. WISHLIST ADD\n");
    printf("13. WISHLIST SWAP <i> <j>\n");
    printf("14. WISHLIST REMOVE <i>\n");
    printf("15. WISHLIST REMOVE\n");
    printf("16. WISHLIST CLEAR\n");
    printf("17. WISHLIST SHOW\n");
    printf("18. LOGOUT\n");
    printf("19. SAVE <filename>\n");
    printf("20. EXIT\n");
    printf("21. HELP\n");
    printf("22. BIOWEAPON\n");
    printf("<<< BACK \n");
    printf("=========================================\n");
}

void workChallengeList() {
    clearterminal();
    printf("____    __    ____  ______   .______       __  ___      ______  __    __       ___       __       __       _______ .__   __.   _______  _______ \n");
    printf("\\   \\  /  \\  /   / /  __  \\  |   _  \\     |  |/  /     /      ||  |  |  |     /   \\     |  |     |  |     |   ____||  \\ |  |  /  _____||   ____|\n");
    printf(" \\   \\/    \\/   / |  |  |  | |  |_)  |    |  '  /     |  ,----'|  |__|  |    /  ^  \\    |  |     |  |     |  |__   |   \\|  | |  |  __  |  |__   \n");
    printf("  \\            /  |  |  |  | |      /     |    <      |  |     |   __   |   /  /_\\  \\   |  |     |  |     |   __|  |  . `  | |  | |_ | |   __|  \n");
    printf("   \\    /\\    /   |  `--'  | |  |\\  \\----.|  .  \\     |  `----.|  |  |  |  /  _____  \\  |  `----.|  `----.|  |____ |  |\\   | |  |__| | |  |____ \n");
    printf("    \\__/  \\__/     \\______/  | _| `._____||__|\\__\\     \\______||__|  |__| /__/     \\__\\ |_______||_______||_______||__| \\__|  \\______| |_______|\n");
    printf("                                                                                                                                                 \n");
    printf("\n=========================================\n");
    printf("           WORK CHALLENGE LIST            \n");
    printf("=========================================\n");
    printf(" 1. TEBAK ANGKA (PLAYING COST = 200)\n");
    printf(" 2. W0RDL3 (PLAYING COST = 500)\n");
    printf(" 3. QUANTUM W0RDL3 (PLAYING COST = 750)\n");
    printf("<<< KELUAR\n");
    printf("=========================================\n");
}

void artTebak()
{
    printf(".___________. _______ .______        ___       __  ___         ___      .__   __.   _______  __  ___      ___      \n");
    printf("|           ||   ____||   _  \\      /   \\     |  |/  /        /   \\     |  \\ |  |  /  _____||  |/  /     /   \\     \n");
    printf("`---|  |----`|  |__   |  |_)  |    /  ^  \\    |  '  /        /  ^  \\    |   \\|  | |  |  __  |  '  /     /  ^  \\    \n");
    printf("    |  |     |   __|  |   _  <    /  /_\\  \\   |    <        /  /_\\  \\   |  . `  | |  | |_ | |    <     /  /_\\  \\   \n");
    printf("    |  |     |  |____ |  |_)  |  /  _____  \\  |  .  \\      /  _____  \\  |  |\\   | |  |__| | |  .  \\   /  _____  \\  \n");
    printf("    |__|     |_______||______/  /__/     \\__\\ |__|\\__\\    /__/     \\__\\ |__| \\__|  \\______| |__|\\__\\ /__/     \\__\\ \n");
    printf("                                                                                                                   \n");
    printf("\n");
}

void artBio()
{
    printf(".______    __    ______   ____    __    ____  _______     ___      .______     ______   .__   __. \n");
    printf("|   _  \\  |  |  /  __  \\  \\   \\  /  \\  /   / |   ____|   /   \\     |   _  \\   /  __  \\  |  \\ |  | \n");
    printf("|  |_)  | |  | |  |  |  |  \\   \\/    \\/   /  |  |__     /  ^  \\    |  |_)  | |  |  |  | |   \\|  | \n");
    printf("|   _  <  |  | |  |  |  |   \\            /   |   __|   /  /_\\  \\   |   ___/  |  |  |  | |  . `  | \n");
    printf("|  |_)  | |  | |  `--'  |    \\    /\\    /    |  |____ /  _____  \\  |  |      |  `--'  | |  |\\   | \n");
    printf("|______/  |__|  \\______/      \\__/  \\__/     |_______/__/     \\__\\ | _|       \\______/  |__| \\__| \n");
    printf("                                                                                                   \n");
    printf("\n");
}

void artWordl()
{
    printf("____    __    ____  ___   .______       _______   __       ____   \n");
    printf("\\   \\  /  \\  /   / / _ \\  |   _  \\     |       \\ |  |     |___ \\  \n");
    printf(" \\   \\/    \\/   / | | | | |  |_)  |    |  .--.  ||  |       __) | \n");
    printf("  \\            /  | | | | |      /     |  |  |  ||  |      |__ <  \n");
    printf("   \\    /\\    /   | |_| | |  |\\  \\----.|  '--'  ||  `----. ___) | \n");
    printf("    \\__/  \\__/     \\___/  | _| `._____||_______/ |_______||____/  \n");
    printf("                                                                  \n");
    printf("\n");
}

void artWork()
{
    printf("____    __    ____  ______   .______       __  ___ \n");
    printf("\\   \\  /  \\  /   / /  __  \\  |   _  \\     |  |/  / \n");
    printf(" \\   \\/    \\/   / |  |  |  | |  |_)  |    |  '  /  \n");
    printf("  \\            /  |  |  |  | |      /     |    <   \n");
    printf("   \\    /\\    /   |  `--'  | |  |\\  \\----.|  .  \\  \n");
    printf("    \\__/  \\__/     \\______/  | _| `._____||__|\\__\\ \n");
    printf("                                                    \n");
    printf("\n");
}

void artSList()
{
    printf("     _______.___________.  ______   .______       _______     __       __       _______.___________.\n");
    printf("    /       |           | /  __  \\  |   _  \\     |   ____|   |  |     |  |     /       |           |\n");
    printf("   |   (----`---|  |----`|  |  |  | |  |_)  |    |  |__      |  |     |  |    |   (----`---|  |----`\n");
    printf("    \\   \\       |  |     |  |  |  | |      /     |   __|     |  |     |  |     \\   \\       |  |     \n");
    printf(".----)   |      |  |     |  `--'  | |  |\\  \\----.|  |____    |  `----.|  | .----)   |      |  |     \n");
    printf("|_______/       |__|      \\______/  | _| `._____||_______|   |_______||__| |_______/       |__|     \n");
    printf("                                                                                                     \n");
    printf("\n");
}

void artSRmv()
{
    printf("     _______.___________.  ______   .______       _______    .______       _______ .___  ___.   ______   ____    ____  _______ \n");
    printf("    /       |           | /  __  \\  |   _  \\     |   ____|   |   _  \\     |   ____||   \\/   |  /  __  \\  \\   \\  /   / |   ____|\n");
    printf("   |   (----`---|  |----`|  |  |  | |  |_)  |    |  |__      |  |_)  |    |  |__   |  \\  /  | |  |  |  |  \\   \\/   /  |  |__   \n");
    printf("    \\   \\       |  |     |  |  |  | |      /     |   __|     |      /     |   __|  |  |\\/|  | |  |  |  |   \\      /   |   __|  \n");
    printf(".----)   |      |  |     |  `--'  | |  |\\  \\----.|  |____    |  |\\  \\----.|  |____ |  |  |  | |  `--'  |    \\    /    |  |____ \n");
    printf("|_______/       |__|      \\______/  | _| `._____||_______|   | _| `._____||_______||__|  |__|  \\______/      \\__/     |_______|\n");
    printf("                                                                                                                                \n");
    printf("\n");
}

void artSSply()
{
    printf("     _______.___________.  ______   .______       _______         _______. __    __  .______   .______    __      ____    ____ \n");
    printf("    /       |           | /  __  \\  |   _  \\     |   ____|       /       ||  |  |  | |   _  \\  |   _  \\  |  |     \\   \\  /   / \n");
    printf("   |   (----`---|  |----`|  |  |  | |  |_)  |    |  |__         |   (----`|  |  |  | |  |_)  | |  |_)  | |  |      \\   \\/   /  \n");
    printf("    \\   \\       |  |     |  |  |  | |      /     |   __|         \\   \\    |  |  |  | |   ___/  |   ___/  |  |       \\_    _/   \n");
    printf(".----)   |      |  |     |  `--'  | |  |\\  \\----.|  |____    .----)   |   |  `--'  | |  |      |  |      |  `----.    |  |     \n");
    printf("|_______/       |__|      \\______/  | _| `._____||_______|   |_______/     \\______/  | _|      | _|      |_______|    |__|     \n");
    printf("                                                                                                                                \n");
    printf("\n");
}

void artSReq()
{
    printf("     _______.___________.  ______   .______       _______    .______       _______   ______      __    __   _______     _______.___________.\n");
    printf("    /       |           | /  __  \\  |   _  \\     |   ____|   |   _  \\     |   ____| /  __  \\    |  |  |  | |   ____|   /       |           |\n");
    printf("   |   (----`---|  |----`|  |  |  | |  |_)  |    |  |__      |  |_)  |    |  |__   |  |  |  |   |  |  |  | |  |__     |   (----`---|  |----`\n");
    printf("    \\   \\       |  |     |  |  |  | |      /     |   __|     |      /     |   __|  |  |  |  |   |  |  |  | |   __|     \\   \\       |  |     \n");
    printf(".----)   |      |  |     |  `--'  | |  |\\  \\----.|  |____    |  |\\  \\----.|  |____ |  `--'  '--.|  `--'  | |  |____.----)   |      |  |     \n");
    printf("|_______/       |__|      \\______/  | _| `._____||_______|   | _| `._____||_______| \\_____\\_____\\\\______/  |_______|_______/       |__|     \n");
    printf("                                                                                                                                             \n");
    printf("\n");
}